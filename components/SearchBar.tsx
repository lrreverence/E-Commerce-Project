    ...
    <Dialog open={showSearch} onOpenChange={() => setShowSearch(!showSearch)}>
      ...
      <DialogContent className='max-w-5xl min-h-[90vh] max-h-[90vh] flex flex-col overflow-hidden'>
        ...
        <div className='w-full h-full overflow-y-scroll border border-darkColor/20 rounded-md '>
          <div>
            {loading ? <p className='flex items-center px-6 py-10 gap-1 text-center text-yellow-600 font-semibold'>
              <Loader2 className='h-5 w-5 animate-spin'/>
            Searching in progress...</p>:products.length ? (
            products?.map((product:Product)=>(
              <div key={product?._id} className='bg-white overflow-hidden border-b last:border-b-0'>
                <div className='flex items-center p-1'>
                <Link href={`/product/${product?.slug?.current}`} className='h-20 w-20 md:h-24 md:w-24 flex-shrink-0 border border-darkColor/20 rounded-md overflow-hidden group' onClick={()=> setShowSearch(false)}>
